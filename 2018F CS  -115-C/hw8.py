...
def listOfRunLengths(s):
    ...
    def lORL(s, result, curCount, curVal):
        ...
        if s=="": return result+[curCount]
        elif s[0] == curVal: return lORL(s[1:],result,curCount+1,s[0])
        else: return lORL(s[1:],result+[curCount],1,s[0])
...
def findRunBits(s):
    ...
    runLens = listOfRunLengths(s)
    avg = sum(runLens)/len(runLens)
    return min(math.ceil(math.log(avg,2))+1,7) # TO DO 
