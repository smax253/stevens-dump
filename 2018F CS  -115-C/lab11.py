...
def insertV2(L, i):
    ...
    index = search(L, i, L[i])
    #print(index)
    for swap in range(i-index):
        current = i-swap
        temp = L[current]
...
def letterCounts(fname):
    ...
    for line in file:
        for entry in line:
            if letter(entry) not in counts:
                counts[letter(entry)] = 1
            else: counts[letter(entry)] += 1
    #print (counts)
    templist = []
    for entry in counts.keys():
        templist += [(counts[entry],entry)]
    #print(templist)
    ...
    for i in range(len(templist)):
        finalList += [templist[len(templist)-1-i]]
    return finalList
