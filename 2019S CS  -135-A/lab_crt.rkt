...
(define (M tuplelist)
  (if (null? tuplelist)
      1
      (* (cadar tuplelist) (M(cdr tuplelist)))))
...
(define (CRT-exists-helper n tuplelist)
  (if(null? tuplelist)
     #t
     (if(equal? (gcd n (cadar tuplelist)) 1)
        (CRT-exists-helper n (cdr tuplelist))
        #f)))

(define (CRT-exists? tuplelist)
  (if (null? tuplelist)
      #t
      (if (CRT-exists-helper (cadar tuplelist) (cdr tuplelist))
...
(define (CRT-helper tuplelist m)
  (if (null? tuplelist)
      0
      (+ (* (caar tuplelist) (/ m (cadar tuplelist)) (mul_inv (/ m (cadar tuplelist)) (cadar tuplelist))) (CRT-helper (cdr tuplelist) m))))
...
(define (CRT-fix crt m)
  (cond
    ((negative? crt) (CRT-fix (+ crt m) m))
    ((> (- crt m) 0) (CRT-fix (- crt m) m))
    (else crt))
  )
(define (CRT tuplelist)
  (if(CRT-exists? tuplelist)
     (CRT-fix (CRT-helper tuplelist (M tuplelist)) (M tuplelist))
     -1))
